...
type RunState = {
  loading?: boolean;
  done?: { summary: string; count: number };
  error?: string;
};
...
export default function ActionCenter({ analysis }: { analysis: Analysis }) {
  ...
  async function fire(key: ActionKey) {
    setRuns((s) => ({ ...s, [key]: { loading: true } }));
    try {
      const res = await fetch(`/api/actions/execute?key=${key}`, {
        method: "POST",
      });
      ...
      setRuns((s) => ({
        ...s,
        [key]: { done: { summary: data.summary, count: data.count } },
      }));
    } catch (e) {
      setRuns((s) => ({
        ...s,
        ...
        {actions.map((action) => (
          ...
          >
            ...
            <button
              ...
              disabled={runs[action.key]?.loading}
              className="mt-2 w-full rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-4 py-2 text-sm font-medium text-emerald-300 transition-colors hover:bg-emerald-500/20 disabled:opacity-60"
            >
              {runs[action.key]?.loading
                ? "Executing…"
                : runs[action.key]?.done
                  ? "✓ Executed (simulated)"
                  : "Execute (simulated)"}
            </button>

            {runs[action.key]?.done && (
              <p className="mt-2 text-xs leading-snug text-emerald-400/90">
                {runs[action.key]!.done!.summary}
              </p>
            )}
            {runs[action.key]?.error && (
              <p className="mt-2 text-xs leading-snug text-rose-400">
                {runs[action.key]!.error}
              </p>
            )}
          </div>
